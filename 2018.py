...
def loadDB(date):
    ...
    for row in c1.execute('SELECT * from runners').fetchall():
        ...
        c2 = db.cursor()
#        c2.execute('SELECT COUNT(*) FROM log WHERE runnerid = ? AND week = ?', (runnerid, week))
#        res = c2.fetchone()
#        print('--------', res[0])
        c2.execute('INSERT OR REPLACE INTO log (runnerid, week, distance) VALUES (?, ?, ?)', (runnerid, week, dist[1]))
        c2.execute('SELECT * FROM log WHERE runnerid = ? AND week = ?', (runnerid, week))
        res = c2.fetchone()
