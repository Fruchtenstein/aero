...
for t in teams:
    tgoal = c1.execute('SELECT sum(goal) FROM runners WHERE teamid=? AND isill=0', (t[0],)).fetchone()[0]/52
    runners = c1.execute('SELECT runnerid FROM runners WHERE teamid=? AND isill=0', (t[0],)).fetchall()
    tmileage = 0
    tpercentage = 0
    for r in runners:
        rmileage = c1.execute('SELECT sum(distance) FROM log WHERE runnerid=? AND week=?', (r[0], week)).fetchone()[0]
...
#tbl = sorted(tbl, key=lambda x: x[4], reverse = True)
#output = []
...
#    output.append('                 <tr{}><td>{}</td><td>{:.2f}</td><td>{:.2f}</td><td>{:.2f}</td><td>{}</td></tr>'.format(alt, t[1], t[2], t[3], t[4], pts))
#    c1.execute('INSERT OR REPLACE INTO tlog VALUES (?, ?, ?, ?)', (t[0], week, pts, t[4]))
#    odd = not odd
#output.append('               </tbody>')
...
#tbl=[]
#for t in teams:
#    pts = c1.execute('SELECT SUM(pts) FROM tlog WHERE teamid=?',(t[0],)).fetchone()[0]
#    tbl.append([t[1], pts])
#
